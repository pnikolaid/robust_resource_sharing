from parameters import slices, zones, freqs, days, T, test_scenario, start_times
...
for data_index in range(slices):
    ...
    while True:
        try:
            ...
            # Find current day
            first_timestamp = plist1_dl[0][0]
            first_timedate = datetime.fromtimestamp(first_timestamp, ZoneInfo(key='Europe/Madrid'))
            current_day = 0
            for i in range(days):
                reference_day = start_times[data_index] + timedelta(i)
                if first_timedate.date() == reference_day.date():
                    current_day = i + 1
                    break
            ...
            if current_day < days:
                print(f"Appending batch {first_timedate} to trial phase list")
                trial_list1_dl += plist1_dl
            else:
                print(f"Appending batch {first_timedate} to regular phase list")
